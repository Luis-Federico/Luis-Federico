...
static gboolean draw(GtkWidget* widget,cairo_t* context)
{
	...
	char buffer1[17] = "Aceleration : ";
int aceleration=(int)s;
	char speed [3];
	if (aceleration==-1){speed[0]='-';speed[1]='1';speed[2]='\0';}
	if (aceleration==1)
	{speed[0]='+';speed[1]='1';speed[2]='\0';}
	if(aceleration==0){speed[0]=' ';speed[1]='0';speed[2]='\0';}
...
GtkWidget *make_box(const gboolean homogeneous,
	 ...
	 * expand. */
	if (expand == TRUE) {
		button4 = gtk_button_new_with_label ("SPEED");
		g_signal_connect (G_OBJECT (button4), "clicked", G_CALLBACK(on_button_click), (gpointer) "4");
	}else{
		button4 = gtk_button_new_with_label ("VELOCITY");
		g_signal_connect (G_OBJECT (button4), "clicked", G_CALLBACK(on_button_click), (gpointer) "4");
}
	...
	GtkWidget *button5 = gtk_button_new_with_label(fill ? "PARA" : "STOP");
...
/*
	...
	gtk_box_pack_start (GTK_BOX (box2), button, expand, fill, padding);
